...
| 溫層 | 門檻       | 程式碼位置          |
| :--- | :--------- | :------------------ |
| 冷藏 | **$2,000** | `CheckoutModal.tsx` |
| 常溫 | **$800**   | `CheckoutModal.tsx` |
...
| 條件                      | 前端行為                                  | 位置              |
| :------------------------ | :---------------------------------------- | :---------------- |
| `stock === 0`             | 灰階遮罩 + 禁用按鈕 + 「已售完」          | `ProductCard.tsx` |
| `stock > 0 && stock <= 5` | 商品卡片右上角顯示紅色/黃色低庫存警告標籤 | `ProductCard.tsx` |
| `stock > 5`               | 正常顯示，可加入購物車                    | `ProductCard.tsx` |
...
| Phase            | 方式                                    |
| :--------------- | :-------------------------------------- |
| Phase 1 (當前)   | `localStorage('yogo-favorites')` (收藏) |
| Phase 2 (已實作) | Firestore `products/{id}.stock`         |
...
| 參數     | 值             | 位置         |
| :------- | :------------- | :----------- |
| 觸發區域 | Header `.logo` | `Header.tsx` |
| 點擊次數 | **5 次**       | `Header.tsx` |
| 時間視窗 | **2,000 ms**   | `Header.tsx` |
