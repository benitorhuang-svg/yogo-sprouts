export function setupScrollSpyAndSticky() {
  const tabContainer = document.querySelector('.category-tabs') as HTMLElement;
  if (!tabContainer) return;

  const header = document.querySelector('.site-header') as HTMLElement;
  const announcement = document.querySelector('.announcement-bar') as HTMLElement;
  ...
  tabBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (cat === 'favorites') {
        const favs: number[] = JSON.parse(localStorage.getItem('yogo_favorites') || '[]');
        const sections = document.querySelectorAll('.category-section') as NodeListOf<HTMLElement>;
        let totalFavoritesFound = 0;

        sections.forEach(sec => {
          const cards = sec.querySelectorAll('.product-card') as NodeListOf<HTMLElement>;
        ...
        document.querySelectorAll('.tab-btn').forEach(b => {
          b.classList.toggle('active', (b as HTMLElement).dataset.category === 'favorites');
        });
