...
import { PRODUCTS, saveProductsState } from "./data.js";
...
let preferredDeliveryDate: string = '';
...
function renderCheckoutStep() {
  ...
  if (checkoutStep === 1) {
    ...
    const contact = (document.getElementById('cust-contact') as HTMLInputElement)?.value || '';
    const address = (document.getElementById('cust-address') as HTMLTextAreaElement)?.value || '';
    const dDate = (document.getElementById('delivery-date') as HTMLInputElement)?.value || '';
  ...
  if (checkoutStep === 1) {
    ...
    document.getElementById('apply-coupon-btn')?.addEventListener('click', async () => {
      const input = document.getElementById('coupon-code-input') as HTMLInputElement | null;
      if (!input) return;
    ...
    modal.querySelectorAll('.checkout-qty-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const action = (e.currentTarget as HTMLElement).classList.contains('plus') ? 'plus' : 'minus';
    ...
    document.getElementById('checkout-to-step3')?.addEventListener('click', () => {
      const form = document.getElementById('checkout-form-element') as HTMLFormElement;
...
function renderCheckoutItemRow(p: Product): string {
  const qty = cart[p.id];
