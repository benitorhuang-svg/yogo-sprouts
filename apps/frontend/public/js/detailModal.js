...
let modalSlides = [];
export function openDetailModal(product) {
  ...
  `;
    ...
    const activeSlideImg = document.getElementById('active-slide-img');
    ...
    const modalDisplay = document.getElementById('modal-qty-display');
    const modalAddBtn = document.getElementById('modal-add-btn');
    function updateModalQtyControls() {
        if (modalMinus)
            modalMinus.disabled = tempQty <= 1;
        if (modalPlus)
            modalPlus.disabled = tempQty >= product.stock;
        if (modalDisplay)
            modalDisplay.textContent = tempQty.toString();
    }
    ...
    modalAddBtn?.addEventListener('click', () => {
        ...
        const cardMinus = document.querySelector(`.product-card[data-id="${product.id}"] .btn-minus`);
        if (cardMinus)
            cardMinus.disabled = false;
